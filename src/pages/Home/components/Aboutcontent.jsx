      ...
      <svg viewBox="0 0 900 340" className="w-full h-full" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <g transform="translate(10, 60)" filter="url(#softGlow)">
          ...
          {[0, 1, 2].map(col => [0, 1, 2, 3].map(row => (
            <rect key={`k${col}${row}`}
              x={14 + col * 30} y={98 + row * 26} width="20" height="14" rx="4"
              fill="#374151" opacity={0.8 - row * 0.1} />
          )))}
          {[0, 1, 2, 3, 4].map(i => (
            <circle key={i} cx={14 + i * 16} cy={202} r="2" fill="#9ca3af" opacity="0.5" />
          ))}
          {[1, 2, 3].map(n => (
            <circle key={`rn${n}`} cx="55" cy="170" r="10" stroke="#facc15" strokeWidth="1.5" fill="none" opacity="0">
              <animate attributeName="r" from="12" to={20 + 22 * n} dur="2.6s" begin={`${n * 0.7}s`} repeatCount="indefinite" />
              <animate attributeName="opacity" from="0.45" to="0" dur="2.6s" begin={`${n * 0.7}s`} repeatCount="indefinite" />
            </circle>
          ))}
        </g>
        ...
        <g transform="translate(800, 55)" filter="url(#softGlow)">
          <path d="M12 80 Q50 10 88 80" stroke="#facc15" strokeWidth="6" strokeLinecap="round" fill="none" />
          ...
          {[0, 1, 2].map(i => (
            <line key={i} x1="7" y1={80 + i * 8} x2="21" y2={80 + i * 8}
              stroke="#d1d5db" strokeWidth="1.8" opacity="0.7" strokeLinecap="round" />
          ))}
          {[0, 1, 2].map(i => (
            <line key={i} x1="83" y1={80 + i * 8} x2="97" y2={80 + i * 8}
              stroke="#d1d5db" strokeWidth="1.8" opacity="0.7" strokeLinecap="round" />
          ))}
          <path d="M14 108 Q-10 132 -2 148" stroke="#facc15" strokeWidth="3.5" strokeLinecap="round" fill="none" />
          <circle cx="-2" cy="150" r="7" fill="#facc15" opacity="0.9" />
          {[1, 2, 3].map(n => (
            <circle key={n} cx="50" cy="90" r="14" stroke="#facc15" strokeWidth="1.5" fill="none" opacity="0">
              <animate attributeName="r" from="14" to={20 + 26 * n} dur="2.6s" begin={`${n * 0.8}s`} repeatCount="indefinite" />
              <animate attributeName="opacity" from="0.5" to="0" dur="2.6s" begin={`${n * 0.8}s`} repeatCount="indefinite" />
            </circle>
          ))}
        </g>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        <div className="shrink-0 w-14 h-14 rounded-2xl bg-[#0891b2]/10 border border-[#0891b2]/30 flex items-center justify-center text-[#0891b2] group-hover:scale-110 transition-transform duration-300">
...
export default function VoIPLanding() {
  const [form, setForm] = useState({ name: "", phone: "", email: "", company: "", country: "", plan: "", message: "" });
      ...
      <div className="min-h-screen bg-white text-gray-900">
        ...
        <div className="bg-white border-gray-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10 text-center">
            <h1 className="text-2xl sm:text-3xl md:text-4xl mb-6 sm:mb-10">
              Communication that{" "}
        ...
        {/* FEATURES + FORM */}
        {/* Mobile: stacked (features → form) | Desktop: side by side */}
        <div
          className="mx-auto px-4 sm:px-6 py-10 sm:py-16 flex flex-col lg:grid lg:grid-cols-[1.1fr_1.2fr] gap-10 lg:gap-24"
        ...
        >
          ...
          {/* RIGHT — Form */}
          {/* Mobile: natural height (no fixed h-[580px]) | Desktop: sticky + fixed height */}
          <div className="lg:sticky lg:top-8">
            <div className="rounded-3xl overflow-hidden shadow-xl border border-cyan-100 lg:h-[580px]">

              {/* Form Header */}
              <div className="bg-[#0891b2] px-5 sm:px-7 pt-6 sm:pt-7 pb-5">
              ...
              {/* Form Body — scrollable on desktop, natural height on mobile */}
              <div className="bg-white px-5 sm:px-7 py-6 lg:overflow-y-auto lg:h-[calc(580px-120px)]">
                {submitted ? (
