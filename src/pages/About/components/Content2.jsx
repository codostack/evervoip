...
export default function AboutUsContent2() {
  return (
    <section className="w-full bg-white py-16 px-6 md:px-20">
      ...
      {/* ── SECTION 1 ──
          Desktop : [image]  [text]   (grid md:grid-cols-2, image first naturally)
          Mobile  : image → title → description  (flex column)
      */}
      <div className="flex flex-col md:grid md:grid-cols-2 gap-12 items-center mb-20">

        {/* Image — always first in DOM so it's first on mobile too */}
        <img
          src={aboutImg}
          alt="About"
      ...
      {/* ── SECTION 2 ──
          Desktop : [text]  [image]   (text col-1, image col-2)
          Mobile  : image → title → description  (flex column, image moved first via order)
      */}
      <div className="flex flex-col md:grid md:grid-cols-2 gap-12 items-center mb-20">

        {/* Image — on mobile shown FIRST via order-first, on desktop order-last keeps it col-2 */}
        <img
          src={aboutImg2}
          alt="Mission"
          className="
            w-full object-cover rounded-xl
            h-[280px] sm:h-[380px] md:h-[680px] md:mt-[40px]
            order-first md:order-last
          "
        />

        {/* Text — on mobile shown SECOND via order-last, on desktop order-first keeps it col-1 */}
        <div className="max-w-xl w-full order-last md:order-first">
          <h2 className="text-2xl font-default mb-4">
