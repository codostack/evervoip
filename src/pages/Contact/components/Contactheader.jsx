...
const NODES = [
  /* ================= MICROSOFT TEAMS ================= */
  {
    ...
    logo: (
      <svg viewBox="0 0 24 24" width="28" height="28" fill="#6264A7">
        ...
        <text x="5" y="15" fontSize="8" fill="#fff" fontWeight="700">T</text>
      </svg>
    ),
  },
  {
    id: 2, name: "Instagram", color: "#C13584", border: "#ffd6e7",
    pos: { x: 50, y: 9 },
    logo: (
  ...
  {
    id: 3, name: "LinkedIn", color: "#0A66C2", border: "#c8dff8",
    pos: { x: 84, y: 18 },
    logo: (
  ...
  {
    id: 6, name: "Facebook", color: "#1877F2", border: "#bfcffa",
    pos: { x: 20, y: 83 },
    logo: (
  ...
  {
    id: 7, name: "Google", color: "#4285F4", border: "#c8d8fb",
    pos: { x: 50, y: 91 },
    logo: (
...
const CX = 50, CY = 50; // center % position
const SIZE = 420;        // internal SVG coordinate space

function pct(v) { return (v / 100) * SIZE; }

/* ── Connector line + travelling dot ── */
function Connection({ node, idx }) {
  const pathId = `cp${node.id}`;
  const x1 = pct(CX), y1 = pct(CY);
  const x2 = pct(node.pos.x), y2 = pct(node.pos.y);
  ...
  return (
    <>
      ...
      <section style={{
        ...
        {/* Ambient orbs */}
        {[
          { w: 360, h: 360, top: -100, left: -100, c: "#dbeafe", d: "0s", dur: "7s" },
          { w: 220, h: 220, bottom: -70, left: -50, c: "#fce7f3", d: "1s", dur: "9s" },
          { w: 200, h: 200, top: -50, right: 50, c: "#d1fae5", d: "0.6s", dur: "8s" },
          { w: 170, h: 170, bottom: -50, right: -30, c: "#fef3c7", d: "2s", dur: "6s" },
        ].map((o, i) => (
          <div key={i} style={{
        ...
        <div style={{
          ...
          backgroundSize: "28px 28px", opacity: .22, pointerEvents: "none",
        }} />
